...
function Usuarios() {
    var columns = [
        ...
        { title: 'E-mail', field: 'email' },
        {
            ...
            field:
                'rol' /*, render: rowData => {
        if(rowData['rol'].length === 0) {
            return 'Sin rol'
        }

        return rowData['rol'][0]['_id']
        //return rowData['rol'].map(rol => rol['name']).join(', ')
    }*/,
            lookup: {
                '60f4bbe737eea54b04cf92ea': 'Estudiante',
                '60f8d78ef58ab5e915cd6434': 'Ayudante',
            },
        },
        /*{
          title: 'Tipo',
    ...
    const listUsers = () => {
        api.get('/usuario', { headers: headers })
            .then((res) => {
                ...
                setData(
                    res.data.usuarios.map((usuario) => {
                        usuario.rol = usuario.rol.map((rol) => rol['_id']);
                        return usuario;
                    })
                );
            })
    ...
    const handleRowUpdate = (newData, oldData, resolve) => {
        ...
        if (errorList.length < 1) {
            api.patch('/usuario/' + newData._id, newData, { headers })
                .then(() => {
    ...
    const handleRowAdd = (newData, resolve) => {
        ...
        if (errorList.length < 1) {
            api.post('/usuario', newData, { headers: headers })
                .then(() => {
                    let dataToAdd = [...data];
    ...
    const handleRowDelete = (oldData, resolve) => {
        ...
        api.delete('/usuario/' + oldData._id, { headers: headers })
            .then(() => {
                ...
                <Card>
                    <CardHeader color="primary">
                        ...
                        <p className={classes.cardCategoryWhite}>
                            En esta seccion puede crear editar y eliminar
